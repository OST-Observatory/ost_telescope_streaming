...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f'flat_capture_{datetime.now().strftime("%Y%m%d")}.log', encoding="utf-8"
            ),
        ],
    )
...
Examples:
  ...
  python flat_capture_runner.py --config config_flat_capture.yaml --skip-warmup
    ...
    parser.add_argument(
        "--num-flats", type=int, help="Number of flat frames to capture (overrides config)"
    )
    ...
    parser.add_argument("--tolerance", type=float, help="Count rate tolerance (overrides config)")
    ...
    try:
        ...
        _global_flat_capture = flat_capture
        ...
        min_exp = flat_config["min_exposure"]
        max_exp = flat_config["max_exposure"]
