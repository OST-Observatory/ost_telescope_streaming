...
def initialize_cooling(config, logger):
    ...
    try:
        ...
        if camera_type == "ascom":
            ...
            camera = ASCOMCamera(
                driver_id=ascom_config.get("driver_id"), config=config, logger=logger
            )
        ...
        if wait_for_cooling:
            ...
            stabilization_status = cooling_manager.wait_for_stabilization(timeout=cooling_timeout)

            if not stabilization_status.is_success:
                logger.warning(f"Temperature stabilization: {stabilization_status.message}")
                return camera, warning_status(
                    f"Cooling initialized but stabilization failed: {stabilization_status.message}"
                )
            else:
    ...
    def signal_handler(signum, frame):
        ...
        try:
            ...
            if global_cooling_manager and not args.skip_warmup and not args.masters_only:
                ...
                if warmup_status.is_success:
                    logger.info("🔥 Warmup initiated (non-blocking). Exiting soon...")
                    logger.info("This avoids blocking on interrupt during shutdown.")
                else:
                    logger.warning(f"Warmup start: {warmup_status.message}")
