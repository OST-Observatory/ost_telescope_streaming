...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f'dark_capture_{datetime.now().strftime("%Y%m%d")}.log', encoding="utf-8"
            ),
        ],
    )
...
Examples:
  ...
  python dark_capture_runner.py --config config_dark_capture.yaml --skip-warmup
    ...
    parser.add_argument(
        "--num-darks", type=int, help="Number of dark frames per exposure time (overrides config)"
    )
    ...
    parser.add_argument(
        "--bias-only", action="store_true", help="Capture only bias frames (minimum exposure time)"
    )
    ...
    try:
        ...
        video_capture = VideoCapture(
            config=config, logger=logger, enable_calibration=False, return_frame_objects=True
        )
        ...
        if not args.skip_cooling:
            ...
            if hasattr(video_capture, "camera") and video_capture.camera:
                global_cooling_manager = create_cooling_manager(
                    video_capture.camera, config, logger
                )
                ...
                if wait_for_cooling:
                    ...
                    stabilization_status = global_cooling_manager.wait_for_stabilization(
        ...
        _global_dark_capture = dark_capture
        ...
        min_exp = dark_config["min_exposure"]
        max_exp = dark_config["max_exposure"]
        logger.info("  Exposure range: %.3fs - %.1fs", min_exp, max_exp)
