...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f'master_frames_{datetime.now().strftime("%Y%m%d")}.log', encoding='utf-8')
        ]
    )
...
Examples:
  ...
  python master_frame_runner.py --config config_master_frames.yaml --debug
    ...
    parser.add_argument(
        '--darks-only',
        action='store_true',
        help='Create only master darks'
    )
    
    parser.add_argument(
        '--flats-only',
        action='store_true',
        help='Create only master flats'
    )
    ...
    parser.add_argument(
        '--sigma-threshold',
        type=float,
        help='Sigma threshold for rejection (overrides config)'
    )
