...
def parse_header_string(header_string: str) -> tuple[str, Any, Optional[str]]:
    ...
    if '=' not in header_string:
        raise ValueError(f"Invalid header format: {header_string}. Use 'KEYWORD=value' or 'KEYWORD=value,comment'")
...
def add_headers_to_fits(file_path: Path, headers_to_add: List[tuple], headers_to_remove: List[str], 
                       backup: bool = False, logger: logging.Logger = None) -> bool:
...
def list_headers(fits_files: List[Path], logger: logging.Logger = None) -> None:
    ...
    for file_path in fits_files:
        try:
            with fits.open(file_path) as hdul:
                ...
                important_headers = ['EXPTIME', 'GAIN', 'XBINNING', 'YBINNING', 'CCD-TEMP', 
                                   'DATE-OBS', 'CAMERA', 'COLORTYP', 'BAYERPAT']
...
Examples:
  ...
  python add_fits_headers.py --directory . --add "TELESCOP=80mm APO" "INSTRUMENT=ASI2600MC" "OBSERVER=Astronomer"
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Header keywords to remove'
    )
        ...
        if args.add:
            for header_string in args.add:
                try:
                    ...
                    logger.info(f"Will add: {keyword}={value}" + (f" ({comment})" if comment else ""))
