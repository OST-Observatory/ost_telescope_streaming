...
def camera_info(camera_config: dict) -> str:
    ...
    return f"Camera: {str(camera_type).upper()} ({sensor_width}×{sensor_height}mm, {pixel_size}μm, {bit_depth}bit)"
...
def calculate_secondary_fov(config: dict) -> Tuple[float, float]:
    ...
    if fov_type == 'camera':
        ...
        return (sensor_w / focal_length) * (180.0 / np.pi), (sensor_h / focal_length) * (180.0 / np.pi)
