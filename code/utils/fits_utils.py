...
def enrich_header_from_metadata(header,  # fits.Header
    ...
    if 'EXPTIME' not in header:
        if hasattr(camera, 'exposure_time'):
            header['EXPTIME'] = getattr(camera, 'exposure_time')
        else:
            camera_config = config.get_camera_config()
            cfg = camera_config.get('ascom', {}) if camera_type == 'ascom' else camera_config.get('alpaca', {})
    ...
    if gain is not None:
        ...
        header['GAIN'] = getattr(camera, 'gain')
    ...
    if offset is not None:
        ...
        header['OFFSET'] = getattr(camera, 'offset')
    else:
        camera_config = config.get_camera_config()
    ...
    if readout is not None:
        ...
        header['READOUT'] = getattr(camera, 'readout_mode')
    else:
        camera_config = config.get_camera_config()
        for sub in ('ascom', 'alpaca'):
            cfg = camera_config.get(sub, {})
    ...
    try:
        ...
        if hasattr(camera, 'set_ccd_temperature'):
            tset = getattr(camera, 'set_ccd_temperature')
            if tset is not None:
                header['CCD-TSET'] = float(tset)
        if hasattr(camera, 'cooler_power'):
            cpwr = getattr(camera, 'cooler_power')
            if cpwr is not None:
                header['COOLPOW'] = float(cpwr)
        if hasattr(camera, 'cooler_on'):
            header['COOLERON'] = bool(getattr(camera, 'cooler_on'))
    except Exception:
        pass
