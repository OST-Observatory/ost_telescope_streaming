...
class AlpycaCameraWrapper:
    ...
    def connect(self):
        ...
        try:
            self.logger.info(
                f"Connecting to Alpyca camera at {self.host}:{self.port}, device {self.device_id}"
            )
    ...
    def set_cooling(self, target_temp, keep_connection=False):
        ...
        try:
            ...
            self.logger.info(
                "Current status - Temp: %s°C, Power: %s%%, Cooler on: %s",
                current_temp,
                current_power,
                current_cooler_on,
            )
            ...
            self.logger.info(
                "New status - Temp: %s°C, Power: %s%%, Cooler on: %s, Target: %s°C",
                new_temp,
                new_power,
                new_cooler_on,
                new_target,
    ...
    def wait_for_cooling_stabilization(self, timeout=60, check_interval=2.0):
        ...
        try:
            ...
            # Timeout reached
            final_info = (
                self.get_cooling_status().data if self.get_cooling_status().is_success else {}
            )
