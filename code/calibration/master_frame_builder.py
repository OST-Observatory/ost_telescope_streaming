...
class MasterFrameCreator:
    ...
    def __init__(self, config=None, logger=None):
        ...
        self.rejection_method = master_config.get(
            "rejection_method", "sigma_clip"
        )  # 'sigma_clip' or 'minmax'
        self.sigma_threshold = master_config.get("sigma_threshold", 3.0)
        self.normalization_method = master_config.get(
    ...
    def create_all_master_frames(self) -> Status:
        ...
        try:
            ...
            return success_status(
                ...
                data={"master_darks": dark_result.data, "master_flats": flat_result.data},
                details={
        ...
        try:
            ...
            for exp_dir in exposure_dirs:
                ...
                if result.is_success:
                    ...
                    self.logger.warning(
                        f"⚠️ Failed to create master dark for {exposure_time:.3f}s: {result.message}"
                    )

            self.logger.info(
                f"Master dark creation completed: {len(created_masters)} masters created"
            )
            ...
            return success_status(
                ...
                details={
                    ...
                    "exposure_times": [
                        self._extract_exposure_time(os.path.basename(m)) for m in created_masters
                    ],
                },
            )
    ...
    def _create_master_dark_for_exposure(self, exp_dir: str, exposure_time: float) -> Status:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(dark_files)} dark files for {exposure_time:.3f}s exposure"
            )
                ...
                with fits.open(file_path) as hdul:
                    ...
                    if exp is not None:
                        try:
                            exp_val = float(exp)
                            self.logger.info(
                                "Detected flat exposure from header %s: %.3fs",
    ...
    def _combine_frames_streaming_files(
        self, files: List[str], frame_type: str, rejection_method: str, sigma_threshold: float
    ) -> Optional[np.ndarray]:
    ...
    def _sigma_clip_combine_files(
        self, files: List[str], sigma: float, shape: Tuple[int, int]
    ) -> Optional[np.ndarray]:
    ...
    def _minmax_combine_files(
        self, files: List[str], shape: Tuple[int, int]
    ) -> Optional[np.ndarray]:
    ...
    def _save_as_fits(
        self, data: np.ndarray, file_path: str, exposure_time: float, frame_type: str
    ) -> bool:
