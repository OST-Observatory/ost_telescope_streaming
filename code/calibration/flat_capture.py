...
class FlatCapture:
    ...
    def __init__(self, config=None, logger=None):
        ...
        self.flat_output_dir = (
            flat_config.get("output_dir") or flat_config.get("output_directory") or "flats"
        )
    ...
    def initialize(self, video_capture: VideoCapture) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Flat capture initialized with target count rate: {self.target_count_rate:.1%}"
            )
            return True
    ...
    def capture_flats(self) -> Status:
        ...
        try:
            ...
            self.logger.info(
                "Starting flat capture: %d frames, target count rate: %.1f%%",
                self.num_flats,
                self.target_count_rate * 100,
            )
            ...
            self.logger.error("Error during flat capture: %s", e)
    ...
    def _adjust_exposure_for_target(self) -> Status:
        ...
        try:
            ...
            for attempt in range(self.max_adjustment_attempts):
                ...
                self.logger.info(
                    "Attempt %d: Exposure=%.3fs, Count rate=%.1f%%, Target=%.1f%%",
                    attempt + 1,
                    self.current_exposure,
                    current_count_rate * 100,
                    self.target_count_rate * 100,
                )
                ...
                if len(previous_count_rates) >= 4:
                    oscillation_detected = self._detect_oscillation(
                        previous_count_rates, previous_exposures
                    )
                    if oscillation_detected:
                        self.logger.warning(
                            "🔄 Oscillation detected! Using conservative adjustment..."
                        )

                # Calculate adaptive step size
                step_factor = self._calculate_adaptive_step(
                    relative_distance, oscillation_detected, attempt
                )
                ...
                if new_exposure > self.max_exposure:
                    self.logger.warning(
                        f"Exposure would exceed maximum ({self.max_exposure}s), using maximum"
                    )
                    new_exposure = self.max_exposure
                elif new_exposure < self.min_exposure:
                    self.logger.warning(
                        f"Exposure would be below minimum ({self.min_exposure}s), using minimum"
                    )
                    new_exposure = self.min_exposure

                # Log the adjustment
                change_percent = (
                    (new_exposure - self.current_exposure) / self.current_exposure
                ) * 100
                self.logger.info(
                    "Adjusting exposure: %.3fs → %.3fs (%+.1f%%, %s, step_factor=%.2f)",
                    self.current_exposure,
                    new_exposure,
                    change_percent,
                    direction,
            ...
            return warning_status(
                f"Could not achieve target count rate after {self.max_adjustment_attempts} attempts"
            )
    ...
    def _calculate_adaptive_step(
        self, relative_distance: float, oscillation_detected: bool, attempt: int
    ) -> float:
    ...
    def _detect_oscillation(self, count_rates: list, exposures: list) -> bool:
        ...
        if oscillation:
            self.logger.debug(
                "Oscillation indicators: crossings=%d, alternating_exposures=%d",
                crossings,
                alternating_exposures,
            )
    ...
    def _capture_test_frame(self) -> Status:
        ...
        try:
            ...
            if isinstance(frame_status.data, np.ndarray):
                ...
                self.logger.debug(
                    f"Using frame data directly: shape={frame.shape}, dtype={frame.dtype}"
                )
            elif hasattr(frame_status.data, "data") and isinstance(
                frame_status.data.data, np.ndarray
            ):
                # Handle nested Status objects
                frame = frame_status.data.data
                self.logger.debug(
                    "Extracted frame from Status object: shape=%s, dtype=%s",
                    frame.shape,
                    frame.dtype,
                )
            elif (
                hasattr(frame_status.data, "data")
                and hasattr(frame_status.data.data, "data")
                and isinstance(frame_status.data.data.data, np.ndarray)
            ):
                # Handle double-nested Status objects
                frame = frame_status.data.data.data
                self.logger.debug(
                    "Extracted frame from double-nested Status object: shape=%s, dtype=%s",
                    frame.shape,
                    frame.dtype,
                )
                ...
                self.logger.debug(
                    "Converted list to numpy array: shape=%s, dtype=%s",
                    frame.shape,
                    frame.dtype,
                )
            else:
                ...
                while hasattr(current_data, "data") and depth < 5:  # Prevent infinite recursion
                    ...
                    if isinstance(current_data, np.ndarray):
                        frame = current_data
                        self.logger.debug(
                            "Found numpy array at depth %d: shape=%s, dtype=%s",
                            depth,
                            frame.shape,
                            frame.dtype,
                        )
                        ...
                        self.logger.debug(
                            "Found list at depth %d, converted to numpy array: shape=%s, dtype=%s",
                            depth,
                            frame.shape,
                            frame.dtype,
                        )
    ...
    def _analyze_frame(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(
                "Frame stats: mean=%.2f, max=%.2f, bd=%d, max=%d, cr=%.1f%%",
                float(mean_value),
                float(max_value),
                int(bit_depth),
                int(max_possible_count),
                float(count_rate * 100),
            )

            # Cap count rate at 100% for saturation
            if count_rate > 1.0:
                self.logger.warning(
                    "Count rate %.1f%% exceeds 100%%, capping at 100%% (saturation)",
                    count_rate * 100,
                )
                count_rate = 1.0
    ...
    def _is_within_tolerance(self, current: float, target: float) -> bool:
        ...
        """
        tolerance_range = target * float(self.count_tolerance)
    ...
    def _capture_flat_series(self) -> Status:
        ...
        """
        try:
            self.logger.info("Capturing %d flat frames...", self.num_flats)
            ...
            for i in range(self.num_flats):
                ...
                if (
                    ...
                    self.logger.warning(
                        f"Failed to capture flat {i+1}: No capture method available"
                    )
                    continue
                ...
                if frame_status.is_success:
                    ...
                    if save_status.is_success:
                        ...
                        self.logger.debug(
                            "Captured flat %d/%d: %s (exposure: %.3fs)",
                            i + 1,
                            self.num_flats,
                            filename,
                            self.current_exposure,
            ...
            self.logger.info(
                f"Flat series capture completed: {len(captured_files)}/{self.num_flats} frames"
            )
