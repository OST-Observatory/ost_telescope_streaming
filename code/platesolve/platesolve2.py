...
class PlateSolve2Automated:
    ...
    def _prepare_parameters(self, ra_deg: Optional[float], dec_deg: Optional[float], fov_width_deg: Optional[float], fov_height_deg: Optional[float]) -> tuple[float, float, float, float]:
        ...
        if self.verbose:
            self.logger.info(f"Parameters: RA={math.degrees(ra_rad):.4f}°, Dec={math.degrees(dec_rad):.4f}°, FOV={fov_width_deg:.4f}°x{fov_height_deg:.4f}°")
    ...
    def _build_command_string(self, image_path: str, ra_rad: float, dec_rad: float, fov_width_rad: float, fov_height_rad: float) -> str:
