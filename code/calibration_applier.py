...
class CalibrationApplier:
    ...
    def __init__(self, config=None, logger=None):
        ...
        self.calibration_tolerance = master_config.get(
            "calibration_tolerance", 0.1
        )  # 10% tolerance
    ...
    def _load_master_frames(self) -> None:
        ...
        try:
            ...
            if not bias_path.exists():
                ...
                candidates = sorted(
                    master_dir.glob("master_bias_*.fits"),
            ...
            for source_dir in dark_sources:
                for dark_file in source_dir.glob("master_dark_*.fits"):
                    try:
                        # Extract exp time from filename: master_dark_1.000s_YYYYMMDD_*.fits
                        ...
                        if m:
                            ...
                            self.logger.info(
                                f"Loaded master dark for {exposure_time}s: {dark_file}"
                            )
                        else:
                            self.logger.warning(
                                f"Unexpected master dark filename format: {dark_file.name}"
                            )
            ...
            if not flat_path.exists():
                candidates = sorted(
                    master_dir.glob("master_flat_*.fits"),
                    key=lambda p: p.stat().st_mtime,
                    reverse=True,
                )
                if candidates:
            ...
            total_masters = (
                (1 if self.master_bias_cache else 0)
            ...
            raise CalibrationError(f"Error loading master frames: {e}") from e
    ...
    def _find_best_master_dark(
        ...
        nearest_dark: Optional[Dict[str, Any]] = None
        nearest_exp_diff = float("inf")

        for dark_info in self.master_dark_cache.values():
            # Check exposure time match
            try:
                exp_diff = abs(float(dark_info.get("exposure_time", 0.0)) - float(exposure_time))
            except Exception:
            ...
            score = (
                exp_diff * 10.0  # Exposure time weight: 10
                + gain_diff * 1.0  # Gain weight: 1
                + offset_diff * 1.0  # Offset weight: 1
                + readout_diff * 1.0
            )  # Readout mode weight: 1
        ...
        if best_match is not None:
            self.logger.debug(
                f"Found master dark: {best_match['file']} "
        ...
        if nearest_dark is not None:
            self.logger.warning(
                "No master dark within tolerance for exp=%.3fs; using nearest exp %.3fs (Δ=%.3fs)",
                exposure_time,
                nearest_dark["exposure_time"],
                nearest_exp_diff,
            )
            return nearest_dark

        self.logger.warning(
            "No suitable master dark found for exp=%s, gain=%s, offset=%s, readout=%s",
            str(exposure_time),
            str(gain),
            str(offset),
            str(readout_mode),
    ...
    def _find_best_master_flat(
        ...
        flat_info: Dict[str, Any] = cast(Dict[str, Any], self.master_flat_cache)
        ...
        if gain_match and offset_match and readout_match:
            self.logger.debug(
                f"Using master flat: {flat_info['file']} "
            ...
            self.logger.warning(
                f"Master flat settings don't match: "
        ...
        try:
            ...
            try:
                ...
                if exposure_time is None and hasattr(frame_data, "metadata"):
                    meta = frame_data.metadata or {}
                    if isinstance(meta, dict):
                        exposure_time = (
                            meta.get("exposure_time_s")
                            or meta.get("exposure_time")
                            or exposure_time
                        )
            except Exception:
                pass
            ...
            if not self.enable_calibration:
                ...
                return success_status(
                    "Calibration disabled", data=frame_data, details={"calibration_applied": False}
                )
            ...
            self.logger.debug(
                "Calibrating frame with exp=%s, gain=%s, offset=%s, readout=%s",
                str(exposure_time),
                str(gain),
                str(offset),
                str(readout_mode),
            ...
            try:
                ...
                if oriented:
                    self.logger.debug(
                        "Standardized frame orientation to long-side horizontal before calibration"
                    )
            except Exception as e_orient:
                self.logger.debug(f"Orientation standardization skipped: {e_orient}")
            try:
                self.logger.debug(
                    "Frame array ok: shape=%s, dtype=%s, min=%s, max=%s",
                    calibrated_frame.shape,
                    calibrated_frame.dtype,
                    (float(np.nanmin(calibrated_frame)) if calibrated_frame.size else "n/a"),
                    (float(np.nanmax(calibrated_frame)) if calibrated_frame.size else "n/a"),
                )
            except Exception:
                pass
            calibration_details = {
            ...
            if master_dark:
                ...
                if dark_data is None:
                    self.logger.warning(
                        f"Selected master dark has no data: {master_dark.get('file')}"
                    )
                else:
                    try:
                        ...
                        self.logger.warning(
                            f"Could not convert master dark to float32 array: {conv_err}"
                        )
                        dark_arr = None
                    if dark_arr is not None:
                        self.logger.debug(
                            "Master dark selected: file=%s, exp=%s, shape=%s, dtype=%s",
                            master_dark.get("file"),
                            master_dark.get("exposure_time"),
                            getattr(dark_arr, "shape", None),
                            getattr(dark_arr, "dtype", None),
                        )
                        ...
                        if dark_arr.shape != calibrated_frame.shape:
                            if dark_arr.ndim == 2 and calibrated_frame.ndim == 2:
                                if dark_arr.T.shape == calibrated_frame.shape:
                                    self.logger.debug(
                                        "Transposing master dark to match frame orientation (2D)"
                                    )
                                    dark_arr = dark_arr.T
                                elif calibrated_frame.T.shape == dark_arr.shape:
                                    self.logger.debug(
                                        "Transposing frame to match master dark orientation (2D)"
                                    )
                                    calibrated_frame = calibrated_frame.T
                        if dark_arr.shape == calibrated_frame.shape:
                            ...
                            try:
                                self.logger.debug(
                                    "After dark subtraction: shape=%s, dtype=%s, min=%s, max=%s",
                                    calibrated_frame.shape,
                                    calibrated_frame.dtype,
                                    (
                                        float(np.nanmin(calibrated_frame))
                                        if calibrated_frame.size
                                        else "n/a"
                                    ),
                                    (
                                        float(np.nanmax(calibrated_frame))
                                        if calibrated_frame.size
                                        else "n/a"
                                    ),
                                )
                            ...
                            self.logger.warning(
                                "Master dark shape %s != frame shape %s; skipping dark subtraction",
                                dark_arr.shape,
                                calibrated_frame.shape,
                            )
                calibration_details["dark_subtraction_applied"] = bool(
                    "applied_dark" in locals() and applied_dark
                )
                ...
                self.logger.warning(
                    f"No suitable master dark found for exp={exposure_time:.3f}s, "
            ...
            if not isinstance(calibrated_frame, np.ndarray):
                self.logger.error(
                    "Calibrated frame has invalid type before flat correction: %s",
                    type(calibrated_frame),
                )
                try:
                    calibrated_frame = np.asarray(calibrated_frame, dtype=np.float32)
                    self.logger.debug(
                        "Recovered calibrated frame to ndarray: shape=%s, dtype=%s",
                        calibrated_frame.shape,
                        calibrated_frame.dtype,
                    )
                except Exception as conv_err:
            ...
            try:
                self.logger.debug(
                    "Pre-flat: frame shape=%s, dtype=%s, min=%s, max=%s",
                    calibrated_frame.shape,
                    calibrated_frame.dtype,
                    (float(np.nanmin(calibrated_frame)) if calibrated_frame.size else "n/a"),
                    (float(np.nanmax(calibrated_frame)) if calibrated_frame.size else "n/a"),
            ...
            if master_flat:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not convert master flat to float32 array: {conv_err}"
                    )
                    flat_data = None
                if flat_data is None:
                    self.logger.warning(
                        f"Selected master flat has no data: {master_flat.get('file')}"
                    )
                    flat_data_safe = None
                else:
                    self.logger.debug(
                        "Master flat selected: file=%s, shape=%s, dtype=%s",
                        master_flat.get("file"),
                        flat_data.shape,
                        flat_data.dtype,
                    )
                    ...
                    if flat_data.shape != calibrated_frame.shape:
                        if flat_data.ndim == 2 and calibrated_frame.ndim == 2:
                            if flat_data.T.shape == calibrated_frame.shape:
                                self.logger.debug(
                                    "Transposing master flat to match frame orientation (2D)"
                                )
                                flat_data = flat_data.T
                            elif calibrated_frame.T.shape == flat_data.shape:
                                self.logger.debug(
                                    "Transposing frame to match master flat orientation (2D)"
                                )
                                calibrated_frame = calibrated_frame.T
                    flat_data_safe = None
                    if flat_data.shape == calibrated_frame.shape:
                        ...
                        try:
                            self.logger.debug(
                                "Flat safe stats: min=%s, max=%s",
                                (
                                    float(np.nanmin(flat_data_safe))
                                    if flat_data_safe.size
                                    else "n/a"
                                ),
                                (
                                    float(np.nanmax(flat_data_safe))
                                    if flat_data_safe.size
                                    else "n/a"
                                ),
                            )
                ...
                if flat_data_safe is not None:
                    ...
                    try:
                        self.logger.debug(
                            "After flat: shape=%s, dtype=%s, min=%s, max=%s",
                            calibrated_frame.shape,
                            calibrated_frame.dtype,
                            (
                                float(np.nanmin(calibrated_frame))
                ...
                self.logger.warning(
                    f"No suitable master flat found for gain={gain}, "
                    f"offset={offset}, readout={readout_mode}"
                )

            # Determine if calibration was applied
            calibration_applied = (
                calibration_details["dark_subtraction_applied"]
                or calibration_details["flat_correction_applied"]
            )
            ...
            if calibration_applied:
                self.logger.info(
                    f"Frame calibrated successfully: "
            ...
            return success_status(
                "Frame calibration completed", data=calibrated_frame, details=calibration_details
            )
    ...
    def get_calibration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_exposure_times": (
                list(self.master_dark_cache.keys()) if self.master_dark_cache else []
            ),
    ...
    def get_master_frame_info(self) -> Dict[str, Any]:
        ...
        return {"master_bias": bias_info, "master_darks": dark_info, "master_flat": flat_info}
