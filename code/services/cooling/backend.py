...
class CoolingManager:
    ...
    def set_target_temperature(self, target_temp: float) -> Status:
        try:
            ...
            return success_status(f"Cooling set to {target_temp}°C", details={
    ...
    def wait_for_stabilization(self, timeout: Optional[int] = None, tolerance: Optional[float] = None) -> Status:
        ...
        try:
            ...
            while (datetime.now() - start).total_seconds() < timeout:
                ...
                self.logger.info(f"Temp: {current_temp:.1f}°C, Target: {self.camera.set_ccd_temperature:.1f}°C, Diff: {diff:+.1f}°C")
                if diff <= tolerance:
                    return success_status(f"Temperature stabilized at {current_temp:.1f}°C", details={'final_temp': current_temp})
        except Exception as e:
            return error_status(f"Error during temperature stabilization: {e}")
        return warning_status("Temperature stabilization timeout", details={'target': self.camera.set_ccd_temperature})
