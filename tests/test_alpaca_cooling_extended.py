...
class CoolingMonitor:
    ...
    def start_monitoring(self, duration_minutes=5):
        ...
        self.data = []
        self.monitor_thread = threading.Thread(target=self._monitor_loop, args=(duration_minutes,))
            ...
            try:
                ...
                if len(self.data) > 1:
                    ...
                    if power > prev_power + 5:  # 5% increase
                        self.logger.info(
                            f"🎉 Cooling power increased from {prev_power:.1f}% to {power:.1f}%!"
                        )
                ...
                if temp <= target + 1.0:  # Within 1°C of target
                    self.logger.info(
                        "🎯 Target temperature reached! Current: %.1f°C, Target: %.1f°C",
                        temp,
                        target,
                    )
                    break
    ...
    def get_summary(self):
        ...
        max_power_time = next(
            point["elapsed_seconds"] for point in self.data if point["power"] == max_power
        )
...
def test_connection_persistence(camera, logger, test_duration_minutes=3):
    ...
    try:
        print("Testing connection persistence during cooling...")
        ...
        it = summary["initial_temp"]
        ft = summary["final_temp"]
        dt = summary["temp_change"]
        print(f"Temperature: {it:.1f}°C → {ft:.1f}°C (Δ{dt:+.1f}°C)")
        ip = summary["initial_power"]
        fp = summary["final_power"]
        dp = summary["power_change"]
        print(f"Power: {ip:.1f}% → {fp:.1f}% (Δ{dp:+.1f}%)")
        ...
        if summary["max_power"] > 0:
            ...
            print("⚠️  Cooling power remained at 0%")
...
def test_connection_stability(camera, logger):
    ...
    try:
        print("Testing connection stability with repeated operations...")
