...
def test_alpaca_connection(config, logger):
    ...
    try:
        ...
        print("Testing Alpyca connection...")
...
def test_continuous_capture(config, logger, duration=30):
    ...
    try:
        ...
        print("✅ Continuous capture test completed")
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["connection", "capture", "continuous", "platesolve", "overlay", "all"],
        default="all",
        help="Specific test to run",
    )
    parser.add_argument(
        "--duration", type=int, default=30, help="Duration for continuous capture test (seconds)"
    )
    ...
    try:
        ...
        if camera_type != "alpaca":
            print(
                f"⚠️  Configuration uses {camera_type} camera, but this test is designed for Alpyca"
            )
            print("   Consider using --camera-type alpaca in your configuration")
