...
def diagnose_cooling_power(camera, logger):
    ...
    try:
        ...
        target_temp = camera.camera.SetCCDTemperature if hasattr(camera.camera, 'SetCCDTemperature') else None
        current_temp = camera.camera.CCDTemperature if hasattr(camera.camera, 'CCDTemperature') else None
...
def main():
    ...
    print(f"=== COOLING POWER DIAGNOSIS ===")
