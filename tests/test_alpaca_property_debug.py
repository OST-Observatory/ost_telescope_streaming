...
def test_cooling_properties(camera):
    ...
    properties = [
        ...
        ("SetCCDTemperature", lambda: camera.set_ccd_temperature, lambda x: setattr(camera, 'set_ccd_temperature', x), -10.0),
...
def test_readout_properties(camera):
    ...
    properties = [
        ("ReadoutMode", lambda: camera.readout_mode, lambda x: setattr(camera, 'readout_mode', x), 0),
        ("ReadoutModes", lambda: camera.readout_modes, None, None),
        ("CanFastReadout", lambda: camera.can_fast_readout, None, None),
        ("FastReadout", lambda: camera.fast_readout, lambda x: setattr(camera, 'fast_readout', x), False),
    ]
...
def test_direct_alpyca_properties(camera):
    ...
    properties_to_test = [
        ...
        "SensorName", "SensorType", "CameraXSize", "CameraYSize",
        "CanSetCCDTemperature", "CanGetCoolerPower",
        "CCDTemperature", "SetCCDTemperature", "CoolerOn", "CoolerPower"
    ]
...
def main():
    ...
    try:
        ...
        if all_failed_properties:
            ...
            unknown_properties = [p for p, e in all_failed_properties if "Property Unknown is not implemented" in e]
            if unknown_properties:
                print(f"\n🎯 SPECIFIC 'Property Unknown' ERRORS:")
                for prop_name, error in unknown_properties:
                    print(f"  🔍 {prop_name}: {error}")
        else:
            print(f"✅ No problematic properties found!")
