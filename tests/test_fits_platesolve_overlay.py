...
def extract_fits_parameters(fits_path: str, logger) -> Dict[str, Any]:
    ...
    try:
        ...
        with fits.open(fits_path) as hdul:
                ...
                if ra_key in header:
                    ...
                    try:
                        ...
                        if ':' in ra_str:
                            # HH:MM:SS format
                            parts = ra_str.split(':')
                            ra_deg = float(parts[0]) * 15 + float(parts[1]) * 0.25 + float(parts[2]) * 0.0041667
                        else:
                            ra_deg = float(ra_str)
                        break
            ...
            logger.info(f"Extracted parameters from FITS:")
                ...
                if len(data.shape) == 3 and data.shape[2] >= 3:
                    ...
                    for channel in range(min(3, data.shape[2])):
                        ...
                        if channel_max > channel_min:
                            normalized_data[:, :, channel] = ((channel_data - channel_min) / (channel_max - channel_min) * 255).astype(np.uint8)
                        else:
                            normalized_data[:, :, channel] = np.full_like(channel_data, 128, dtype=np.uint8)
...
def perform_plate_solving(fits_path: str, parameters: Dict[str, Any], config, logger) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if result.is_success:
            ...
            logger.info(f"  FOV: {result.data.get('fov_width', 'Unknown')}° x {result.data.get('fov_height', 'Unknown')}°")
            return result.data
        else:
...
def generate_overlay(plate_solve_result: Dict[str, Any], parameters: Dict[str, Any], 
                    output_path: str, config, logger) -> bool:
...
def combine_overlay_with_image(image_path: str, overlay_path: str, combined_path: str, logger) -> bool:
...
def test_fits_platesolve_overlay(fits_path: str, output_dir: str, 
    ...
    try:
        ...
        if not generate_overlay(plate_solve_result, parameters, str(png_overlay_path), config, logger):
            logger.error("Failed to generate overlay")
            return False
        
        # Step 5: Combine overlay with image
        logger.info("Step 5: Combining overlay with image...")
        if not combine_overlay_with_image(str(png_image_path), str(png_overlay_path), str(png_combined_path), logger):
...
def test_color_camera_functionality(fits_path: str, output_dir: str, config=None, logger=None) -> bool:
    ...
    try:
        ...
        if not is_color_camera and image_channels == 1:
            ...
            logger.info(f"✅ Color camera detected: {bayer_pattern} pattern, {image_channels} channels")
        else:
        ...
        if combine_overlay_with_image(str(input_png_path), str(overlay_png_path), str(combined_png_path), logger):
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", "-o", default="fits_output", 
    ...
    parser.add_argument("--fov-width", type=float, help="FOV width in degrees (overrides FITS header)")
    parser.add_argument("--fov-height", type=float, help="FOV height in degrees (overrides FITS header)")
    
    # Test mode options
    parser.add_argument("--color-test", action="store_true", 
                       help="Run color camera functionality test")
