...
def parse_test_args(description="Test script"):
    ...
    parser.add_argument("--driver", "-d", help="ASCOM driver ID (overrides config)")
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable verbose logging (DEBUG level)"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug logging (same as --verbose)"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Enable quiet logging (WARNING level only)"
    )
